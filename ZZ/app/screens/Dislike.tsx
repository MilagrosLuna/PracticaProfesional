...
function Dislike(): JSX.Element {
  ...
  const addPost = async (imagen: any) => {
    ...
    try {
      ...
      for (const base64 of imagen) {
        const docRef = doc(collection(Firestore_DB, 'PostDislike'));
        batch.set(docRef, {
  ...
  const getAllPictures = async () => {
    ...
    // Recorre las fotos y obtén el nombre del usuario al que pertenece
    for (const docSnapshot of querySnapshot.docs) {
      const postData = docSnapshot.data();
      const userId = postData.uniqueId; // Asumiendo que tienes un campo "uniqueId" en PostLike

      // Consulta el nombre del usuario
      const userDocRef = doc(Firestore_DB, 'users', userId);
      const userDoc = await getDoc(userDocRef);
      const userData = userDoc.data();
      // Combina los datos de la foto y el usuario
      posts.push({id: docSnapshot.id, ...postData, userName: userData?.perfil}); // Asume que el nombre del usuario se almacena en el campo "nombre"
    }
  ...
  const handleVote = async (postId: string, voteType: 'like' | 'dislike') => {
    try {
      const postRef = doc(collection(Firestore_DB, 'PostDislike'), postId);
  ...
  useLayoutEffect(() => {
    navigation.setOptions({
      headerLeft: () => (
        <View>
          <Text style={{color: '#fff'}}>CosasFeas</Text>
        </View>
      ),
      headerRight: () => (
        <View>
    ...
    <View style={styles.container}>
      ...
      <View style={styles.floatingButtonsContainer}>
        ...
        <TouchableOpacity
          ...
          <Icon name="bar-chart" size={24} color="white"></Icon>
        </TouchableOpacity>
      </View>
    </View>
  );
