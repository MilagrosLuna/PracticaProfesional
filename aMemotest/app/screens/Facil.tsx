...
export default function Facil() {
  ...
  const animalImages = [
    require('../../assets/animales/dino.png'),
    require('../../assets/animales/panda.png'),
    require('../../assets/animales/uni.png'),
    require('../../assets/animales/dino.png'),
    require('../../assets/animales/panda.png'),
    require('../../assets/animales/uni.png'),
  ];
  ...
  const addJugador = async () => {
    ...
    const year = fechaActual.getFullYear(); // Obtiene el año (YYYY)
    const month = (fechaActual.getMonth() + 1).toString().padStart(2, '0'); // Obtiene el mes (MM)
    const day = fechaActual.getDate().toString().padStart(2, '0'); // Obtiene el día (DD)

    const fechaSolo = `${year}-${month}-${day}`; // Forma la fecha en formato "YYYY-MM-DD"

    const nombreJugador = Firebase_AUTH?.currentUser?.email?.toUpperCase();
    ...
    if (nombreJugador) {
      try {
        ...
        if (Platform.OS === 'android') {
          ToastAndroid.show('Jugador y tiempos guardados', ToastAndroid.SHORT);
        }
      } catch (e) {
        console.error('Error al agregar el documento: ', e);
      }
    } else {
      console.error('Nombre de jugador no válido');
    }
  };
